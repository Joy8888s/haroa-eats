...
app.post("/api/register", async (req, res) => {
  ...
  if (password.length < 6) {
    return res.status(400).json({
      error: "Password কমপক্ষে 6 character হতে হবে"
    });
  }
...
app.post("/api/orders", auth, (req, res) => {
  ...
  if (
    ...
    menus.some(m => Number(m.restaurant_id) !== restaurantId)
  ) {
    return res.status(400).json({
      error: "Invalid menu"
    });
...
app.patch("/api/orders/:id/status", auth, (req, res) => {
  ...
  /* RIDER */
  if (user.role === "rider") {
    if (Number(order.delivery_id) !== Number(user.id)) {
      return res.status(403).json({
  ...
  if (user.role === "customer") {
    return res.status(403).json({
      error: "Customer cannot change order status"
    });
  }
  ...
  (req, res) => {
    ...
    if (order.delivery_id) {
      return res.status(409).json({
        error: "এই order অন্য rider already গ্রহণ করেছে"
      });
    }
...
app.post(
  ...
  (req, res) => {
    ...
    if (!name || !area) {
      return res.status(400).json({
        error: "Restaurant name এবং area দিন"
      });
    }
...
app.get(
  ...
  (req, res) => {
    ...
    res.json({
      restaurants: db
        .prepare("SELECT COUNT(*) c FROM restaurants")
        .get().c,
